...
function logoutParams(token: JWT): Record<string, string> {
  if (!token.idToken) throw new Error('idToken is required but is undefined.');

  if (!process.env.NEXTAUTH_URL)
    throw new Error('NEXTAUTH_URL is required but is undefined.');
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (token) return sendEndSessionEndpointToURL(token);
